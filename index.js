...
module.exports = async (path) => {
  ...
  while (!replay.atEnd()) {
    ...
    switch (chunkType) {
      ...
      case 2:
        if (
          (chunkType === 2 && !settings.chunkTypes.checkpoints) ||
          (chunkType === 0 && !settings.chunkTypes.header)
        ) {
          size.size -= 8;
          break;
        }
      ...
      case 1:
        ...
        if (isEncrypted) {
          ...
          decompressed = await wasm.decompressUnsafe(
            compressedBuffer,
            decompressedSize
          );
        } else {
    ...
    switch (part.type) {
      ...
      case "chunk":
        ...
        if (part.size !== newBuffer.offset - startOffset) {
          throw Error(
            `Invalid size in type ${part.chunkType}, Expected: ${
              part.size
            } Got: ${newBuffer.offset - startOffset}`
          );
        }
